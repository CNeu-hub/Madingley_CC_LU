###-----------------------------------------------------------------------------------------###
### TITLE:       Percentage change for maximum land use (maxHANPP) scenarios                ###
### DESCRIPTION: Calculates percentage change for maximum land use (maxHANPP) Phase of:     ###
...
#SSP2.6_2100
cohorts_SSP2.6_2100_control <- SSP126_2100_list[[2]]$time_line_cohorts %>% filter(Month > 2280) %>% rowwise() %>% 
  dplyr::mutate(BiomassSum = sum(c(Biomass_FG_0,Biomass_FG_1,Biomass_FG_2,Biomass_FG_3,Biomass_FG_4,Biomass_FG_5,Biomass_FG_6,Biomass_FG_7,Biomass_FG_8)),
  ...
  ungroup() %>%   as.data.frame()
#SSP8.5_2100
cohorts_SSP8.5_2100_control <- SSP585_2100_list[[2]]$time_line_cohorts %>% filter(Month > 2280) %>% rowwise() %>% 
  dplyr::mutate(BiomassSum = sum(c(Biomass_FG_0,Biomass_FG_1,Biomass_FG_2,Biomass_FG_3,Biomass_FG_4,Biomass_FG_5,Biomass_FG_6,Biomass_FG_7,Biomass_FG_8)),
...
#2. Create df for maxHANPP scenarios + sum up monthly biomass of each scenario (overall effect), filter = > Month 2280, last 10 years
#historical
cohorts_historical_maxHANPP <- historical_2014_list[[11]]$time_line_cohorts %>% filter(Month > 2280) %>% rowwise() %>% 
  dplyr::mutate(BiomassSum = sum(c(Biomass_FG_0,Biomass_FG_1,Biomass_FG_2,Biomass_FG_3,Biomass_FG_4,Biomass_FG_5,Biomass_FG_6,Biomass_FG_7,Biomass_FG_8)),
                Scenario="Historical",Group = "maxHANPP",
                Biomass_FG_3 = sum(c(Biomass_FG_3,Biomass_FG_6)),Biomass_FG_4 = sum(c(Biomass_FG_4,Biomass_FG_7)),Biomass_FG_5 = sum(c(Biomass_FG_5, Biomass_FG_8))) %>%
  summarize(Month,Year,Biomass_FG_0,Biomass_FG_1,Biomass_FG_2,Biomass_FG_3,Biomass_FG_4,Biomass_FG_5,BiomassSum,Scenario,Group) %>%
  ungroup() %>%   as.data.frame()  

#SSP2.6_2100
cohorts_SSP2.6_2100_maxHANPP <- SSP126_2100_list[[11]]$time_line_cohorts %>% filter(Month > 2280) %>% rowwise() %>% 
  dplyr::mutate(BiomassSum = sum(c(Biomass_FG_0,Biomass_FG_1,Biomass_FG_2,Biomass_FG_3,Biomass_FG_4,Biomass_FG_5,Biomass_FG_6,Biomass_FG_7,Biomass_FG_8)),
                Scenario="SSP1-2.6",Group = "maxHANPP",
                Biomass_FG_3 = sum(c(Biomass_FG_3,Biomass_FG_6)),Biomass_FG_4 = sum(c(Biomass_FG_4,Biomass_FG_7)),Biomass_FG_5 = sum(c(Biomass_FG_5, Biomass_FG_8))) %>%
  summarize(Month,Year,Biomass_FG_0,Biomass_FG_1,Biomass_FG_2,Biomass_FG_3,Biomass_FG_4,Biomass_FG_5,BiomassSum,Scenario,Group) %>%
  ungroup() %>%   as.data.frame()
#SSP8.5_2100
cohorts_SSP8.5_2100_maxHANPP <- SSP585_2100_list[[11]]$time_line_cohorts %>% filter(Month > 2280) %>% rowwise() %>% 
  dplyr::mutate(BiomassSum = sum(c(Biomass_FG_0,Biomass_FG_1,Biomass_FG_2,Biomass_FG_3,Biomass_FG_4,Biomass_FG_5,Biomass_FG_6,Biomass_FG_7,Biomass_FG_8)),
                Scenario="SSP5-8.5",Group = "maxHANPP",
                Biomass_FG_3 = sum(c(Biomass_FG_3,Biomass_FG_6)),Biomass_FG_4 = sum(c(Biomass_FG_4,Biomass_FG_7)),Biomass_FG_5 = sum(c(Biomass_FG_5, Biomass_FG_8))) %>%
  summarize(Month,Year,Biomass_FG_0,Biomass_FG_1,Biomass_FG_2,Biomass_FG_3,Biomass_FG_4,Biomass_FG_5,BiomassSum,Scenario,Group) %>%
  ungroup() %>%   as.data.frame()

#3. Reorganize data: to create 1 df per climate scenario --> for logRR calculation 
#creates df with control and maxHANPP application observations (can be distinguished with group variable)
#stick control & treatment dataframe for historical
historical <- rbind(cohorts_historical_control,cohorts_historical_maxHANPP)
...
SSP2.6 <- rbind(cohorts_SSP2.6_2100_control,cohorts_SSP2.6_2100_maxHANPP)
unique(SSP2.6$Group)
#calculation of log10 values 
SSP2.6[3:9] <- log10(SSP2.6[3:9])

#stick control & treatment dataframe for SSP8.5
SSP8.5 <- rbind(cohorts_SSP8.5_2100_control,cohorts_SSP8.5_2100_maxHANPP)
unique(SSP8.5$Group)
#calculation of log10 values 
...
###Historical natural ecosystem state vs. maxHANPP application###
#define length for bootstrapping samples (used for strata)
n1 <- length(cohorts_historical_control$Biomass_FG_0)
n2 <- length(cohorts_historical_maxHANPP$Biomass_FG_0)
...
###SSP1-2.6 natural ecosystem state vs. maxHANPP application###
#define length for bootstrapping samples (used for strata)
n1 <- length(cohorts_SSP2.6_2100_control$Biomass_FG_0)
n2 <- length(cohorts_SSP2.6_2100_maxHANPP$Biomass_FG_0)
...
###SSP5-8.5 natural ecosystem state vs. maxHANPP application###
#define length for bootstrapping samples (used for strata)
n1 <- length(cohorts_SSP8.5_2100_control$Biomass_FG_0)
n2 <- length(cohorts_SSP8.5_2100_maxHANPP$Biomass_FG_0)
...
#2. Create df for maxHANPP scenarios + sum up monthly biomass of each scenario (overall effect), filter = > Month 2280, last 10 years
#historical
cohorts_historical_maxHANPP <- historical_2014_list[[13]]$time_line_cohorts %>% filter(Month > 2280) %>% rowwise() %>% 
  dplyr::mutate(BiomassSum = sum(c(Biomass_FG_0,Biomass_FG_1,Biomass_FG_2,Biomass_FG_3,Biomass_FG_4,Biomass_FG_5,Biomass_FG_6,Biomass_FG_7,Biomass_FG_8)),
                Scenario="Historical",Group = "maxHANPP",
                Biomass_FG_3 = sum(c(Biomass_FG_3,Biomass_FG_6)),Biomass_FG_4 = sum(c(Biomass_FG_4,Biomass_FG_7)),Biomass_FG_5 = sum(c(Biomass_FG_5, Biomass_FG_8))) %>%
  summarize(Month,Year,Biomass_FG_0,Biomass_FG_1,Biomass_FG_2,Biomass_FG_3,Biomass_FG_4,Biomass_FG_5,BiomassSum,Scenario,Group) %>%
...
#SSP2.6_2100
cohorts_SSP2.6_2100_maxHANPP <- SSP126_2100_list[[13]]$time_line_cohorts %>% filter(Month > 2280) %>% rowwise() %>% 
  dplyr::mutate(BiomassSum = sum(c(Biomass_FG_0,Biomass_FG_1,Biomass_FG_2,Biomass_FG_3,Biomass_FG_4,Biomass_FG_5,Biomass_FG_6,Biomass_FG_7,Biomass_FG_8)),
                Scenario="SSP1-2.6",Group = "maxHANPP",
                Biomass_FG_3 = sum(c(Biomass_FG_3,Biomass_FG_6)),Biomass_FG_4 = sum(c(Biomass_FG_4,Biomass_FG_7)),Biomass_FG_5 = sum(c(Biomass_FG_5, Biomass_FG_8))) %>%
  summarize(Month,Year,Biomass_FG_0,Biomass_FG_1,Biomass_FG_2,Biomass_FG_3,Biomass_FG_4,Biomass_FG_5,BiomassSum,Scenario,Group) %>%
  ungroup() %>%   as.data.frame()
#SSP8.5_2100
cohorts_SSP8.5_2100_maxHANPP <- SSP585_2100_list[[13]]$time_line_cohorts %>% filter(Month > 2280) %>% rowwise() %>% 
  dplyr::mutate(BiomassSum = sum(c(Biomass_FG_0,Biomass_FG_1,Biomass_FG_2,Biomass_FG_3,Biomass_FG_4,Biomass_FG_5,Biomass_FG_6,Biomass_FG_7,Biomass_FG_8)),
                Scenario="SSP5-8.5",Group = "maxHANPP",
                Biomass_FG_3 = sum(c(Biomass_FG_3,Biomass_FG_6)),Biomass_FG_4 = sum(c(Biomass_FG_4,Biomass_FG_7)),Biomass_FG_5 = sum(c(Biomass_FG_5, Biomass_FG_8))) %>%
  summarize(Month,Year,Biomass_FG_0,Biomass_FG_1,Biomass_FG_2,Biomass_FG_3,Biomass_FG_4,Biomass_FG_5,BiomassSum,Scenario,Group) %>%
...
#create a effect size data frame = base for forest plot
#Insert NAs where functional groups were going extinct
...
#2. Create df for maxHANPP scenarios + sum up monthly biomass of each scenario (overall effect), filter = > Month 2280, last 10 years
#historical
cohorts_historical_maxHANPP <- historical_2014_list[[8]]$time_line_cohorts %>% filter(Month > 2280) %>% rowwise() %>% 
  dplyr::mutate(BiomassSum = sum(c(Biomass_FG_0,Biomass_FG_1,Biomass_FG_2,Biomass_FG_3,Biomass_FG_4,Biomass_FG_5,Biomass_FG_6,Biomass_FG_7,Biomass_FG_8)),
                Scenario="Historical",Group = "maxHANPP",
                Biomass_FG_3 = sum(c(Biomass_FG_3,Biomass_FG_6)),Biomass_FG_4 = sum(c(Biomass_FG_4,Biomass_FG_7)),Biomass_FG_5 = sum(c(Biomass_FG_5, Biomass_FG_8))) %>%
  summarize(Month,Year,Biomass_FG_0,Biomass_FG_1,Biomass_FG_2,Biomass_FG_3,Biomass_FG_4,Biomass_FG_5,BiomassSum,Scenario,Group) %>%
...
#SSP2.6_2100
cohorts_SSP2.6_2100_maxHANPP <- SSP126_2100_list[[8]]$time_line_cohorts %>% filter(Month > 2280) %>% rowwise() %>% 
  dplyr::mutate(BiomassSum = sum(c(Biomass_FG_0,Biomass_FG_1,Biomass_FG_2,Biomass_FG_3,Biomass_FG_4,Biomass_FG_5,Biomass_FG_6,Biomass_FG_7,Biomass_FG_8)),
                Scenario="SSP1-2.6",Group = "maxHANPP",
                Biomass_FG_3 = sum(c(Biomass_FG_3,Biomass_FG_6)),Biomass_FG_4 = sum(c(Biomass_FG_4,Biomass_FG_7)),Biomass_FG_5 = sum(c(Biomass_FG_5, Biomass_FG_8))) %>%
  summarize(Month,Year,Biomass_FG_0,Biomass_FG_1,Biomass_FG_2,Biomass_FG_3,Biomass_FG_4,Biomass_FG_5,BiomassSum,Scenario,Group) %>%
  ungroup() %>%   as.data.frame()
#SSP8.5_2100
cohorts_SSP8.5_2100_maxHANPP <- SSP585_2100_list[[8]]$time_line_cohorts %>% filter(Month > 2280) %>% rowwise() %>% 
  dplyr::mutate(BiomassSum = sum(c(Biomass_FG_0,Biomass_FG_1,Biomass_FG_2,Biomass_FG_3,Biomass_FG_4,Biomass_FG_5,Biomass_FG_6,Biomass_FG_7,Biomass_FG_8)),
                Scenario="SSP5-8.5",Group = "maxHANPP",
                Biomass_FG_3 = sum(c(Biomass_FG_3,Biomass_FG_6)),Biomass_FG_4 = sum(c(Biomass_FG_4,Biomass_FG_7)),Biomass_FG_5 = sum(c(Biomass_FG_5, Biomass_FG_8))) %>%
  summarize(Month,Year,Biomass_FG_0,Biomass_FG_1,Biomass_FG_2,Biomass_FG_3,Biomass_FG_4,Biomass_FG_5,BiomassSum,Scenario,Group) %>%
...
#2. Create df for maxHANPP scenarios + sum up monthly biomass of each scenario (overall effect), filter = > Month 2280, last 10 years
#historical
cohorts_historical_maxHANPP <- historical_2014_list[[10]]$time_line_cohorts %>% filter(Month > 2280) %>% rowwise() %>% 
  dplyr::mutate(BiomassSum = sum(c(Biomass_FG_0,Biomass_FG_1,Biomass_FG_2,Biomass_FG_3,Biomass_FG_4,Biomass_FG_5,Biomass_FG_6,Biomass_FG_7,Biomass_FG_8)),
                Scenario="Historical",Group = "maxHANPP",
                Biomass_FG_3 = sum(c(Biomass_FG_3,Biomass_FG_6)),Biomass_FG_4 = sum(c(Biomass_FG_4,Biomass_FG_7)),Biomass_FG_5 = sum(c(Biomass_FG_5, Biomass_FG_8))) %>%
  summarize(Month,Year,Biomass_FG_0,Biomass_FG_1,Biomass_FG_2,Biomass_FG_3,Biomass_FG_4,Biomass_FG_5,BiomassSum,Scenario,Group) %>%
...
#SSP2.6_2100
cohorts_SSP2.6_2100_maxHANPP <- SSP126_2100_list[[10]]$time_line_cohorts %>% filter(Month > 2280) %>% rowwise() %>% 
  dplyr::mutate(BiomassSum = sum(c(Biomass_FG_0,Biomass_FG_1,Biomass_FG_2,Biomass_FG_3,Biomass_FG_4,Biomass_FG_5,Biomass_FG_6,Biomass_FG_7,Biomass_FG_8)),
                Scenario="SSP1-2.6",Group = "maxHANPP",
                Biomass_FG_3 = sum(c(Biomass_FG_3,Biomass_FG_6)),Biomass_FG_4 = sum(c(Biomass_FG_4,Biomass_FG_7)),Biomass_FG_5 = sum(c(Biomass_FG_5, Biomass_FG_8))) %>%
  summarize(Month,Year,Biomass_FG_0,Biomass_FG_1,Biomass_FG_2,Biomass_FG_3,Biomass_FG_4,Biomass_FG_5,BiomassSum,Scenario,Group) %>%
  ungroup() %>%   as.data.frame()
#SSP8.5_2100
cohorts_SSP8.5_2100_maxHANPP <- SSP585_2100_list[[10]]$time_line_cohorts %>% filter(Month > 2280) %>% rowwise() %>% 
  dplyr::mutate(BiomassSum = sum(c(Biomass_FG_0,Biomass_FG_1,Biomass_FG_2,Biomass_FG_3,Biomass_FG_4,Biomass_FG_5,Biomass_FG_6,Biomass_FG_7,Biomass_FG_8)),
                Scenario="SSP5-8.5",Group = "maxHANPP",
                Biomass_FG_3 = sum(c(Biomass_FG_3,Biomass_FG_6)),Biomass_FG_4 = sum(c(Biomass_FG_4,Biomass_FG_7)),Biomass_FG_5 = sum(c(Biomass_FG_5, Biomass_FG_8))) %>%
  summarize(Month,Year,Biomass_FG_0,Biomass_FG_1,Biomass_FG_2,Biomass_FG_3,Biomass_FG_4,Biomass_FG_5,BiomassSum,Scenario,Group) %>%
...
#export df as csv 
write.csv(maxHANPP,paste0(figpath,sep="/","maxHANPP_Effect.csv"))

#round values
maxHANPP[4:6] <- round(maxHANPP[4:6], digits = 2)
