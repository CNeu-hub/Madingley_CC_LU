###--------------------------------------------------------------------------------------------------------------------------------------------------------------------###
### TITLE:          MADINGLEYR: 1. Climate (Spin-up), 2. Current Land Use (HANPP) 3. Max. Land Use (maxHANPP)                                                          ###
### DESCRIPTION: Runs MadingleyR for different regions / climate datasets of:                                                                                          ###                              
###              "Model-based Impact Analysis of Climate Change and Land-use Intensification on Trophic Networks."                                                     ###
### NOTES:- Can be applied to different regions (spatial extents are below in the code), region has to be defined                                                      ###                                              
###       - Creates Climate scenarios + HANPP (Current land-use intensity) + max land-use intensity scenarios for historical, SSP1-2.6, and SSP5-8.5 climate scenario; ###
###         output is saved as rdata list (stage 1 of list = climate scenario, entry 2 = climate scenario under current land use, last entry of list = climate         ###  
###         scenario under max. land use); length of list depends on time until only 10 % vegetation = left in region                                                  ###
###       - Output = rdata list, is saved in ModelData folder within repository.                                                                                       ###
###       - Climate data can be obtained at the sources provided in the SI, has to be in 0.5degree resolution.                                                         ###
### DATE:        16.09.2022                                                                                                                                            ###
###--------------------------------------------------------------------------------------------------------------------------------------------------------------------###

###Create environment 
library("MadingleyR")
library(raster)
library(terra)

getwd()
#Function to create climate data input with MadingleyR compatible climate data 
source("InsertClimateDataFunction.R") #Source: Hoeks, S. (2022)

#Function to crop spatial raster input of MadingleyR using spatial window #Source: Hoeks, S. (2022)
source("https://raw.githubusercontent.com/SHoeks/RandomMadingleyRFunctions/master/crop_spatial_rasters_to_window.r")

#define spatial extent of region, meeting requirements of MadingleyR (min. longitude, max. longitude, min. latitude, max. latitude)
#spatial_window = c(25.5,28.5,61,69) #96 grid cells, works brilliant, Finland
#spatial_window = c(16,21,-22,-17) #90 grid cells, works brilliant, Namibia 
#spatial_window = c(-69,-61,-3,0) #96 grid cells, works brilliant, Brazil 
spatial_window = c(-1,6,46,49) #84 grid cells, works brilliant, France

#define region
region <- "France"

#create output path for regional output 
regionpath <- paste0(getwd(), sep = "/", "Output/", region, sep="/")

#check if region extent is chosen correctly 
plot_spatialwindow(spatial_window, main = "Study Region in France")

#define no. of replicates (there are some stochastic processes inside the model, so we use replicates & average across them to generate more robust analysis of output)
reps <- 1

################################################################################################################################################################
###Each simulation was run subsequently (climate, current land use, maximum land use), 
###for each climate scenario (historical, SSP1-2.6, SSP5-8.5) in a loop (in order to create the previously defined no. of replicates)
###Here we provide just one default MadingleyR run, with default climate data as an example 

###----------------------------------------------###
###         1.) HISTORICAL CLIMATE SCENARIO      ###
###----------------------------------------------###

for(i in 1:reps) {

  cat("Replicate", i, "historical climate is running\n")
  
#create Madingley model inputs
#sp_inputs_histo_2014 = insert_climate_data("/Users/neumanch/Desktop/Thesis/bpm-madingley/Christians Projects/MadingleyR_Sep_2023/ClimateData/Historical_Averaged_0.5degree/") #here it is possible to insert any climate data matching the default madingleyR requirements in 0.5degree resolution

#Create spatial input 
#Download 0.5 degree Madingley compatible data & create spatial input for model (used in this example)
envDIR = paste0(getwd(), sep = "/", "example/", "0.5_degree_data", sep="/")

sp_inputs = DownloadLoadHalfDegreeInputs(envDIR)

envDIR = paste0(getwd(), sep = "/", "example/", "0.5_degree_data", sep="/")

#here, we just use standard spatial input of madingleyR to provide an example 
sp_inputs_histo_2014 = sp_inputs
chrt_def = madingley_inputs("cohort definition")
stck_def = madingley_inputs("stock definition")
mdl_prms = madingley_inputs("model parameters")

#check if hanpp layer is correctly loaded to madingley
plot(sp_inputs_histo_2014$hanpp, main="HANPP in gC/m^2/year")

#sp_inputs_histo_2014$hanpp = crop(sp_inputs_histo_2014$hanpp,spatial_window)

# crop the raster to spatial window extent
# helps to speed up loading times and makes working with the inputs easier
sp_inputs_histo_2014 = crop_spatial_rasters_to_window(inputs = sp_inputs_histo_2014, spatial_window = spatial_window)
plot(sp_inputs_histo_2014$hanpp, main = "HANPP in Study Region", xlab = "Longitude", ylab = "Latitude")
text(x=5.9, y=48.5, "HANPP [gC/m-2/yr]", xpd=NA, pos=4)

###------------------###
### INITIALIZE MODEL ###
###------------------###

historical_2014 = madingley_init(spatial_window = spatial_window, 
                                 cohort_def = chrt_def,
                                 stock_def = stck_def,
                                 spatial_inputs = sp_inputs_histo_2014, #run model with default climate data = sptl_input  
                                 max_cohort = 100)

###---------------###
### RUN THE MODEL ###
###---------------###
###-------------------------------------------------------###
### Simulation Experiment 1: Climate (Spin-up)            ###
###-------------------------------------------------------###
regionpath <- paste0(getwd(), sep = "/", "example/", "output", sep="/")

###create a list file, where all outputs will be stored
historical_2014_list = list()

historical_2014_list[[1]] = madingley_run(out_dir = regionpath, #path to output directory
                                          madingley_data = historical_2014,
                                          years = 100, # at least 200 years recommended
                                          cohort_def = chrt_def,
                                          stock_def = stck_def,
                                          spatial_inputs = sp_inputs_histo_2014,
                                          model_parameters = mdl_prms,
                                          max_cohort = 100) # at least 500 is recommended 

###-------------------------------------------------------###
### Simulation Experiment 2: Current Land Use (HANPP)     ###
###-------------------------------------------------------###

#run model for additional 200 years with applying hanpp as input raster with gC/m-2/year
historical_2014_list[[2]] = madingley_run(out_dir = regionpath, #path to output directory
                                          madingley_data = historical_2014_list[[1]],
                                          years = 100, # at least 200 years recommended
                                          cohort_def = chrt_def,
                                          stock_def = stck_def,
                                          spatial_inputs = sp_inputs_histo_2014,
                                          model_parameters = mdl_prms,
                                          max_cohort = 100, # at least 500 is recommended 
                                          apply_hanpp = 2)  #apply_hanpp = 2 is to apply hanpp with gC/m-2/year as input


###-------------------------------------------------------###
### Simulation Experiment 3: Maximum Land Use (maxHANPP)  ###
###-------------------------------------------------------###

#calculation of fractional hanpp raster
sp_inputs_histo_2014$hanpp[] = sp_inputs_histo_2014$hanpp[] + abs(min(sp_inputs_histo_2014$hanpp[], na.rm = TRUE)) 
sp_inputs_histo_2014$hanpp[] = sp_inputs_histo_2014$hanpp[] / max(sp_inputs_histo_2014$hanpp[], na.rm = TRUE)  
sp_inputs_histo_2014$hanpp[] = 1-sp_inputs_histo_2014$hanpp[]  # The subtraction 1-HANPP leads to swapped values, this means the lower values are the values with the highest HANPP and the higher values are the values with lowest HANPP --> So, we can assume that a reduction in these values is a reduction in biomass, and not a reduction in HANPP.

#remove NA's
sp_inputs_histo_2014$hanpp[is.na(sp_inputs_histo_2014$hanpp[])] = 0.001

###VEGETATION REDUCTION###
#Create HANPP backup to compare with reduced HANPP later
hanpp_backup = sp_inputs_histo_2014$hanpp

#Reduce vegetation using a while loop, stops when needed (no more values above 0.1 in hanpp raster)
while(max(sp_inputs_histo_2014$hanpp[])>0.1) {
  
  #use ifelse to reduce vegetation by 0.1 per 5 years (if its > 0.1), but not below the threshold 0.1
  sp_inputs_histo_2014$hanpp[] = ifelse(sp_inputs_histo_2014$hanpp[] > 0.1,
                                        ifelse(sp_inputs_histo_2014$hanpp[] - 0.1 < 0.1,0.1,sp_inputs_histo_2014$hanpp[] - 0.1),
                                        sp_inputs_histo_2014$hanpp[])
  plot(sp_inputs_histo_2014$hanpp)
  
  #run the model for 10 years with HANPP reduced by 0.1 (10 %)
  historical_2014_list[[length(historical_2014_list)+1]] = madingley_run(out_dir = regionpath,
                                                                         years = 10, 
                                                                         madingley_data = historical_2014_list[[length(historical_2014_list)]],
                                                                         spatial_inputs = sp_inputs_histo_2014,
                                                                         silenced = TRUE,
                                                                         max_cohort = 100, # at least 500
                                                                         apply_hanpp = 1) 
}

#Compare start HANPP with end HANPP 
par(mfrow=c(1,2))
plot(hanpp_backup,main="Starting HANPP", xlab = "Longitude", "Latitude")
plot(sp_inputs_histo_2014$hanpp,main="HANPP Reduced to Maximum of 0.1", xlab = "Longitude", ylab = "Latitude")
par(mfrow=c(1,1))

###POST VEGETATION REDUCTION###
historical_2014_list[[length(historical_2014_list)+1]]  = madingley_run(out_dir = regionpath,
                                                                        madingley_data = historical_2014_list[[length(historical_2014_list)]], #!!! SH, get last element in list
                                                                        years = 100, #at least 200
                                                                        model_parameters = mdl_prms, 
                                                                        spatial_inputs = sp_inputs_histo_2014,
                                                                        cohort_def = chrt_def,
                                                                        max_cohort = 100, #at least 500
                                                                        apply_hanpp = 1)

#save image for further use in next scripts
save.image(paste0(regionpath %+% i %+% "Historical_Run.Rdata"))

#clear all data frames from environment for next scenario
rm(list=ls(all=TRUE)[sapply(mget(ls(all=TRUE)), class) == "data.frame"])
#clear all lists from environment for next scenario
rm(list=ls(all=TRUE)[sapply(mget(ls(all=TRUE)), class) == "list"])

}



